  ...
  computed: {
    isClickedAll: {
      get() {
        ...
        return (
          this.clickedBulkItemsCount >= this.pagination.state.to - this.pagination.state.from + 1 &&
          this.clickedBulkItemsCount > 0 &&
          this.allClickedItemsAreSame()
        );
      },
      set(clicked) {},
    },
  ...
  methods: {
    allClickedItemsAreSame() {
      ...
      for (let i = 0; i < itemsInPaginationIds.length; i++) {
        ...
        if (
          this.bulkItems[itemInPaginationId] === undefined ||
          this.bulkItems[itemInPaginationId] === false
        ) {
          return false;
        }
      }
    ...
    onBulkItemClicked(id) {
      this.bulkItems[id] === undefined
        ? Vue.set(this.bulkItems, id, true)
        : (this.bulkItems[id] = !this.bulkItems[id]);
    },
    ...
    bulkDelete(url) {
      const itemsToDelete = Object.keys(pickBy(this.bulkItems));
      const self = this;
    ...
    loadData(resetCurrentPage) {
      ...
      axios.get(this.url, options).then(
        (response) => this.populateCurrentStateAndData(response.data.data),
        (error) => {
          // TODO handle error
        }
      );
              ...
              axios.post(url, { publish_now: true }).then(
                function (response) {
                  ...
                  _this.$notify({
                    ...
                    text: response.data.message
                      ? response.data.message
                      : _this.trans[dialogType].success,
                  });
                },
                function (error) {
                  _this.$notify({
    ...
    unpublishNow: function unpublishNow(url, row, additionalWarning) {
      ...
      this.$modal.show("dialog", {
        title: _this.trans[dialogType].title,
        text:
          _this.trans[dialogType].text +
          (additionalWarning
            ? '<br /><span class="text-danger">' + additionalWarning + "</span>"
            : ""),
        buttons: [
          { title: _this.trans[dialogType].no },
          {
      ...
      this.$modal.show(
        ...
        },
        {
          ...
          save: function save(mutablePublishedAt) {
            ...
            axios.post(url, { published_at: mutablePublishedAt }).then(
              function (response) {
                row.published_at = response.data.object.published_at;
                _this.$notify({
                  type: "success",
