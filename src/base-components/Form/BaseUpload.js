...
const BaseUpload = {
  ...
  methods: {
    ...
    onFileDelete: function (file, error, xhr) {
      var deletedFileIndex = this.mutableUploadedImages.findIndex((o) => o.url === file.url);
    ...
    attachAlreadyUploadedMedia: function () {
      this.$nextTick(() => {
        if (this.mutableUploadedImages) {
          for (const [key, file] of Object.entries(this.mutableUploadedImages)) {
            this.$refs[this.collection].manuallyAddFile(
              { name: file["name"], size: file["size"], type: file["type"], url: file["url"] },
              file["thumb_url"],
              false,
              false,
              {
    ...
    getFiles: function () {
      ...
      for (const [key, file] of Object.entries(this.$refs[this.collection].getAcceptedFiles())) {
    ...
    placeIcon: function (file) {
      ...
      if (file.type.includes("image")) {
        ...
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-pdf-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("word")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-word-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("spreadsheet") || file.type.includes("csv")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-excel-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("presentation")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-powerpoint-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("video")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-video-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("text")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-text-o"></i><p>' + file.name + "</p>");
      } else if (file.type.includes("zip") || file.type.includes("rar")) {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-archive-o"></i><p>' + file.name + "</p>");
      } else {
        $previewElement
          .find(".dz-image")
          .html('<i class="fa fa-file-o"></i><p>' + file.name + "</p>");
      }
    },
